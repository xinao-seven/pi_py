...
import { discoverLocalExtensions, serverExtensionDirectory } from "../../src/services/agent-registry.js";
...
describe("server extension discovery", () => {
  ...
  it("discovers the built-in approval extension from the canonical directory", () => {
    const files = discoverLocalExtensions(serverExtensionDirectory(join(process.cwd(), "src", "services")));

    expect(files.map((file) => basename(file))).toEqual(expect.arrayContaining(["tool-approval.ts", "plan-mode.ts"]));
  });
});
