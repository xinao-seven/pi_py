...
describe("McpService（stdio 集成）", () => {
  it("连接 server、注册工具、调用成功/失败工具", async () => {
    ...
    try {
      ...
      const sum = await add.execute("call-2", { a: 2, b: 3 }, undefined, undefined, undefined as never);
      ...
      await expect(fail.execute("call-3", {}, undefined, undefined, undefined as never)).rejects.toThrow(/boom/);
    } finally {
  ...
  it("删除配置后断开连接（sync 对账）", async () => {
    ...
    try {
      ...
      // 已断开：调用应抛 503
      await expect(service.callTool(cwd, "mcp__test_server__echo", { text: "x" })).rejects.toMatchObject({ code: "mcp_not_connected" });
    } finally {
      await service.dispose();
    }
  });
...
describe("MCP 内联扩展（审批联动）", () => {
  it("approval: required 的 server 工具走事件总线审批", async () => {
    ...
    try {
      ...
      const ctx = { hasUI: false, sessionManager: { getSessionId: () => "session-1" }, signal: undefined as AbortSignal | undefined };
  ...
  it("未配置 approval 的 server 工具直接放行", async () => {
    ...
    try {
      ...
      // 非 MCP 工具不拦截
      const bash = await fakePi.runToolCall({ toolName: "bash", toolCallId: "call-2", input: { command: "ls" } }, ctx);
