...
class StubPiSessionFactory implements PiSessionFactory {
  async create(): Promise<PiSession> { return new StubPiSession(); }
}
...
describe("/api/mcp routes", () => {
  it("嵌套 server 配置：新增 → 列表(connected) → 试连 → 启停 → 删除", async () => {
    ...
    try {
      ...
      // 列表：connected + 3 工具
      response = await app.inject({ method: "GET", url: `/api/mcp/servers?cwd=${encodeURIComponent(cwd)}` });
      expect(response.statusCode).toBe(200);
      const servers = response.json().servers;
      expect(servers).toHaveLength(1);
      expect(servers[0]).toMatchObject({ name: "test-server", scope: "workspace", status: "connected", toolCount: 3 });

      // 试连（已保存配置）
      response = await app.inject({ method: "POST", url: "/api/mcp/servers/test-server/test", payload: { cwd } });
      expect(response.statusCode).toBe(200);
      ...
      response = await app.inject({
        ...
        payload: { cwd, server: { transport: "stdio", command: process.execPath, args: [fixturePath] } },
      });
      expect(response.json()).toMatchObject({ success: true, toolCount: 3 });
      ...
      response = await app.inject({
        ...
        payload: { cwd, scope: "workspace", server: { transport: "stdio", command: process.execPath, args: [fixturePath], enabled: false } },
      });
      expect(response.statusCode).toBe(200);
      response = await app.inject({ method: "GET", url: `/api/mcp/servers?cwd=${encodeURIComponent(cwd)}` });
      ...
      // 删除
      response = await app.inject({ method: "DELETE", url: `/api/mcp/servers/test-server?cwd=${encodeURIComponent(cwd)}&scope=workspace` });
      expect(response.statusCode).toBe(200);
      response = await app.inject({ method: "GET", url: `/api/mcp/servers?cwd=${encodeURIComponent(cwd)}` });
