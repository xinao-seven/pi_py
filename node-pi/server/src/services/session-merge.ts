 ...
 */
export function createMergeSummary(
  source: MergeableSessionManager,
  target: MergeableSessionManager,
  sourceId: string,
  ...
  const unique = source
    .getEntries()
    .filter((entry) => typeof entry.id === 'string' && !targetIds.has(entry.id));
  ...
  return {
    content: truncate(lines.join('\n'), MAX_SUMMARY_CHARS, false),
    sourceUniqueEntryCount: unique.length,
 ...
 */
export function appendMergeSummary(
  target: MergeableSessionManager,
  sourceId: string,
...
function describe(entry: Entry): string | undefined {
  if (entry.type === 'message' && object(entry.message)) {
    ...
    if (role === 'assistant' && Array.isArray(entry.message.content)) {
      ...
      return tools.length
        ? `Assistant tool calls: ${[...new Set(tools)].sort().join(', ')}`
        : undefined;
    }
    return undefined;
  }
...
function contentText(content: unknown): string {
  ...
  return content
    .filter(object)
    .filter(
      (block): block is Record<string, string> =>
