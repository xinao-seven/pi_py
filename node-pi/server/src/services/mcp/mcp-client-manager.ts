...
export class McpClientManager {
   ...
   * - 尚未连接且启用 → 连接并发现工具。
   */
  async sync(cwd: string, desired: Array<{ name: string; config: McpServerConfig }>): Promise<void> {
  ...
  async callTool(cwd: string, name: string, toolName: string, args: Record<string, unknown>, signal?: AbortSignal): Promise<CallToolResult> {
  ...
  private async connect(key: string, cwd: string, name: string, config: McpServerConfig): Promise<void> {
  ...
  private async disconnect(key: string): Promise<void> {
    ...
    if (!server) return;
    try { await server.client?.close(); } catch { /* 已断开可忽略 */ }
    try { await server.transport?.close(); } catch { /* 已断开可忽略 */ }
  ...
  private async openAndList(config: McpServerConfig, defaultCwd?: string): Promise<{
    client: Client;
    transport: StdioClientTransport | StreamableHTTPClientTransport;
    tools: Tool[];
  }> {
  ...
  private buildTransport(config: McpServerConfig, defaultCwd?: string): StdioClientTransport | StreamableHTTPClientTransport {
    if (config.transport === "stdio") {
      return new StdioClientTransport({
        command: config.command ?? "",
        args: config.args ?? [],
        // 合并 process.env，保证 npx 等依赖 PATH 的命令可用；用户 env 覆盖同名项。
        env: { ...(process.env as Record<string, string>), ...(interpolateEnvMap(config.env) ?? {}) },
