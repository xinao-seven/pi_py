...
function serializeInfo(
  ...
  return {
    ...
    parentSessionId:
      pathKey(session.parentSessionPath) === undefined
        ? null
 ...
 */
async function reparentAndDelete(
  target: PersistedSessionInfo,
  ...
  for (const child of children) {
    ...
    if (!lines[0])
      throw new ApiError(409, 'session_delete_failed', `Session header is missing: ${child.id}`);
    let header: Record<string, unknown>;
    try {
      header = JSON.parse(lines[0]) as Record<string, unknown>;
    } catch {
      throw new ApiError(409, 'session_delete_failed', `Session header is invalid: ${child.id}`);
    }
...
export const sessionRoutes: FastifyPluginAsync<SessionRouteOptions> = async (app, options) => {
  ...
  app.post<{ Params: { sessionId: string } }>('/:sessionId/fork', async (request) => {
    ...
    if (manager === undefined) {
      throw new ApiError(
        409,
        'fork_unavailable',
        'Session branching is unavailable for this session',
      );
    }
    // createBranchedSession 是 SessionManager 提供的分叉能力：
    // 复制当前会话到新文件并定位到指定叶子，返回新会话文件路径。
    const filePath = (
      manager as typeof manager & { createBranchedSession(leafId: string): string | undefined }
  ...
  app.post<{ Params: { sessionId: string }; Body: { sourceSessionId?: unknown } }>(
    '/:sessionId/merge',
    async (request) => {
      ...
      if (!targetManager || !sourceManager) {
        throw new ApiError(
          409,
          'merge_unavailable',
          'Session merge is unavailable for this session',
        );
      }
      // 生成摘要（源会话里"目标会话没有的条目"的压缩文本）；没有可合并内容则 409。
      const summary = createMergeSummary(sourceManager, targetManager, sourceSessionId);
      if (!summary)
        throw new ApiError(409, 'nothing_to_merge', 'Source session has no mergeable content');
  ...
  // 提供的能力），不改变消息内容。
  app.patch<{ Params: { sessionId: string }; Body: { name?: unknown } }>(
    '/:sessionId',
    async (request) => {
      ...
      const manager = entry.session.sessionManager as
        | (typeof entry.session.sessionManager & { appendSessionInfo(name: string): string })
        | undefined;
      if (!manager)
        throw new ApiError(
          409,
          'rename_unavailable',
          'Session rename is unavailable for this session',
        );
  ...
  app.get<{ Params: { sessionId: string } }>('/:sessionId', async (request) => {
    ...
    const persisted =
      index.sessions.find((session) => session.id === entry.session.sessionId) ??
      entry.persisted ??
      fallbackInfo(entry);
