...
async function testDraft(): Promise<void> {
  ...
  error.value = null;
  try {
    const result = await testMcpServer(draft.value.name.trim() || "server", props.cwd, draft.value.scope, draftToConfig(draft.value));
    testResult.value = result.success
      ? `连接成功：发现 ${result.toolCount} 个工具`
      : `连接失败：${result.error ?? "未知错误"}`;
  } catch (cause) {
...
function statusLabel(status: McpServerView["status"]): string {
  return { connected: "已连接", connecting: "连接中", error: "连接失败", disabled: "已禁用" }[status] ?? status;
}
...
<template>
  <div class="modal-backdrop" @click.self="emit('close')" @keydown.esc="emit('close')">
    <section class="config-dialog config-dialog--wide" role="dialog" aria-modal="true" aria-labelledby="mcp-title">
      <header class="config-header">
        <div>
          <div class="welcome-kicker">MODEL CONTEXT PROTOCOL</div>
          <h2 id="mcp-title">MCP Servers</h2>
          <p>MCP 工具以 <code>mcp__&lt;server&gt;__&lt;tool&gt;</code> 命名出现在会话工具列表；配置保存后自动对所有会话生效。</p>
        </div>
      ...
      <div v-else-if="formOpen && draft" class="config-body">
        <div class="config-presets">
          ...
          <button type="button" class="config-add" :disabled="testing === 'form'" @click="testDraft">
        ...
        <article class="provider-card mcp-form">
          ...
          <template v-if="draft.transport === 'stdio'">
            <div class="provider-grid">
              ...
              <label>参数（空格分隔）<input v-model="draft.args" placeholder="-y @modelcontextprotocol/server-filesystem E:/code" /></label>
            </div>
            <div class="provider-grid">
            ...
            <label class="mcp-line-field">环境变量（每行 KEY=value 或 KEY: value）
              <textarea v-model="draft.env" rows="3" placeholder="API_KEY=$MY_TOKEN" spellcheck="false" />
            </label>
          </template>
          ...
          <template v-else>
            ...
            <label class="mcp-line-field">请求头（每行 Header: value）
              <textarea v-model="draft.headers" rows="3" placeholder="Authorization: Bearer $TOKEN" spellcheck="false" />
            </label>
            <label class="checkbox-field mcp-switch"><input v-model="draft.approval" type="checkbox" />工具调用需人工审批</label>
          </template>

          <p class="config-help mcp-help">stdio 通过 <code>command/args</code> 启动子进程（可执行任意命令），与 bash 同等级信任；建议为有外部副作用的 server 开启人工审批。</p>
        </article>
      ...
      <div v-else class="config-body">
        ...
        <div v-if="servers.length === 0" class="config-empty">尚未配置 MCP server，点击上方添加。</div>
        <article v-for="server in servers" :key="`${server.scope}:${server.name}`" class="skill-card">
          <div>
            <div class="mcp-title-row">
              <strong>{{ server.name }}</strong>
              <span class="mcp-badge mcp-badge--scope">{{ server.scope === "workspace" ? "工作区" : "用户" }}</span>
            ...
            <p v-if="server.status === 'error' && server.error" class="mcp-error">{{ server.error }}</p>
          ...
          <div class="mcp-actions">
            <button type="button" class="mcp-toggle" :class="{ 'mcp-toggle--on': server.enabled }" :aria-pressed="server.enabled" @click="toggleEnabled(server)">
              {{ server.enabled ? "已启用" : "已停用" }}
            </button>
            <button type="button" :disabled="testing === server.name" @click="testConnection(server)">
              {{ testing === server.name ? "测试中…" : "测试" }}
            </button>
            <button type="button" @click="editServer(server)">编辑</button>
            <button type="button" class="danger-link mcp-remove" @click="removeServer(server)">删除</button>
          </div>
        </article>
      </div>

      <p v-if="testResult && !formOpen" class="config-notice mcp-test-result" role="status">{{ testResult }}</p>
      <div v-if="error" class="config-error" role="alert">{{ error }}</div>
      <footer class="config-footer">
        <button type="button" @click="emit('close')">{{ formOpen ? "关闭" : "完成" }}</button>
        <button v-if="formOpen" type="button" class="primary-action" :disabled="saving" @click="save">
...
.mcp-badge--connected .mcp-dot { background: #2ecc71; }
.mcp-badge--connecting .mcp-dot { background: #f1c40f; }
.mcp-badge--error .mcp-dot { background: #e74c3c; }
.mcp-badge--disabled .mcp-dot { background: var(--muted); }
