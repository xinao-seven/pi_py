...
describe("FileService", () => {
  ...
  it("lists and reads registered-workspace files without exposing secrets", async () => {
    ...
    await expect(service.readText(".env", workspace)).rejects.toMatchObject({ code: "sensitive_file" });
    await expect(service.readText("../outside.txt", workspace)).rejects.toMatchObject({ code: "path_outside_workspace" });
  });
});
